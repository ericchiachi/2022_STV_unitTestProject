...
public final class GeoHash {
    ...
    public static String adjacentHash(String hash, Direction direction) {
        ...
        char lastChar = source.charAt(source.length() - 1); // 最後的char
        Parity parity = (source.length() % 2 == 0) ? Parity.EVEN : Parity.ODD; // 長度為奇偶
        String base = source.substring(0, source.length() - 1); // 除了最後一字的hash
     ...
     *            negative).
     * @param toRight
     *            bottom right of the grid in hashes to the right (can be
     *            negative).
     * @param toBottom
     *            bottom right of the grid in hashes to the bottom (can be
     *            negative).
